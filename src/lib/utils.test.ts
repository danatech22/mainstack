...
describe('generateChartData', () => {
  it('should generate the correct number of data points', () => {
    const transactions = [
      { date: '2024-01-01', amount: 100, type: 'deposit' },
    ];
    const result = generateChartData(transactions, '2024-01-01', '2024-01-31', 10);
    expect(result).toHaveLength(10);
  });

  it('should calculate cumulative balance for deposits', () => {
    const transactions = [
      { date: '2024-01-01', amount: 100, type: 'deposit' },
      { date: '2024-01-15', amount: 200, type: 'deposit' },
    ];
    const result = generateChartData(transactions, '2024-01-01', '2024-01-31', 5);

    // First point should have first transaction
    expect(result[0].value).toBeGreaterThanOrEqual(0);
    // Last point should have both transactions
    expect(result[result.length - 1].value).toBeGreaterThanOrEqual(0);
  });

  it('should handle withdrawals by subtracting amounts', () => {
    const transactions = [
      { date: '2024-01-01', amount: 500, type: 'deposit' },
      { date: '2024-01-15', amount: 100, type: 'withdrawal' },
    ];
    const result = generateChartData(transactions, '2024-01-01', '2024-01-31', 5);
  ...
  it('should include date in ISO format', () => {
    const transactions = [{ date: '2024-01-01', amount: 100, type: 'deposit' }];
    const result = generateChartData(transactions, '2024-01-01', '2024-01-31', 3);
  ...
  it('should handle empty transactions array', () => {
    const result = generateChartData([], '2024-01-01', '2024-01-31', 5);
